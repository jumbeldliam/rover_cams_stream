...
async fn main() {
    ...
    let manager_thread = ManagerThread::init(io_ev_recv, logging_send.clone(), comms_recv, framerates_send, encoding_send);
...
impl LoggingThread {
    fn init(recv: std::sync::mpsc::Receiver<String>, framerates: std::sync::mpsc::Receiver<Box<[PipelineFramerates]>>) -> Self {
        Self {
            log: recv,
            framerates,
        }
...
impl CommunicationCommand {
    fn from_data_channel_message(msg: &webrtc::data_channel::data_channel_message::DataChannelMessage) -> Option<Self> {
        None
...
impl <'a> CameraThread<'a> {
    fn init(cam_cmds: std::sync::mpsc::Receiver<CameraCommand>, frame_notif: std::sync::mpsc::Sender<()>, send_frame: std::sync::mpsc::Sender<RawFrame>, logging: std::sync::mpsc::Sender<String>, dev: uvc::Device<'a>, dev_idx: DeviceIndex) -> Self {
        Self {
            logging,
    ...
    fn spin(mut self) {
        ...
        let preferred_format = devh.get_preferred_format(|x, y| {
        ...
        let mut streamh = devh.get_stream_handle_with_format(preferred_format).expect("could not get stream_handle");

        let logging = self.logging;
        let send_frame = self.send_frame;
        let frame_cb = self.raw_frame_cb;

        let stream = streamh.start_stream(move |frame| {
            match RawFrame::from_uvc(frame) {
                Ok(raw) => {
                    send_frame.send(raw).unwrap();
                    frame_cb.send(()).unwrap();
                }
                Err(e) => logging.send(format!("could not conv frame: {:?}", e)).unwrap(),
            }
        }).unwrap();
...
impl EncoderThread {
    fn init(cmds: std::sync::mpsc::Receiver<EncodingCommand>, frame_notif: std::sync::mpsc::Sender<()>, recv_raw_frames: std::sync::mpsc::Receiver<RawFrame>, logging: std::sync::mpsc::Sender<String>, send_enc_frames: std::sync::mpsc::Sender<EncodedFrame>, dev_idx: &DeviceIndex) -> Self {
        //let speed_settings = 
        let enc = rav1e::EncoderConfig { width: 64, height: 96, speed_settings: rav1e::config::SpeedSettings::from_preset(10), low_latency: true, ..Default::default() };
        let cfg = rav1e::Config::default().with_encoder_config(enc.clone()).with_threads(4);
...
impl <T> PipelineManager<T> {
    fn fps(&mut self, now: &Time) -> FpsRates {
        ...
        let raw = if let Some(raw_framerate) = process_framerate(&self.raw_frame_notif, &self.time_since_last_raw, &now) {
        ...
        let enc = if let Some(enc_framerate) = process_framerate(&self.enc_frame_notif, &self.time_since_last_enc, &now) {
    ...
    fn instantiate(cam_cmds: std::sync::mpsc::Sender<CameraCommand>, enc_cmds: std::sync::mpsc::Sender<EncodingCommand>, cam_handle: Option<std::thread::JoinHandle<T>>, enc_handle: Option<std::thread::JoinHandle<T>>, raw_frame_notif: std::sync::mpsc::Receiver<()>, enc_frame_notif: std::sync::mpsc::Receiver<()>) -> Self {
        let now = Time::now();
        Self {
...
fn process_framerate(recv: &std::sync::mpsc::Receiver<()>, last_time: &Time, now: &Time) -> Option<u8> {
...
impl ManagerThread {
    fn init(io_events: std::sync::mpsc::Receiver<DeviceIoEvent>, logging: std::sync::mpsc::Sender<String>, comms_cmds: std::sync::mpsc::Receiver<CommunicationCommand>, framerates: std::sync::mpsc::Sender<Box<[PipelineFramerates]>>, encoding: std::sync::mpsc::Sender<EncodedFrame>) -> Self {
        Self {
            io_events,
            logging,
            comms_cmds,
    ...
    fn get_framerates(&mut self) -> Box<[PipelineFramerates]> {
        ...
        self.current_pipelines
            .iter_mut()
            .map(|(dev, pipeline)| PipelineFramerates::from_idx_and_fps(dev.clone(), pipeline.fps(&now)))
           .collect::<Box<_>>()
    }

    #[inline]
    fn new_camera_thread(&self, cam_cmds: std::sync::mpsc::Receiver<CameraCommand>, frame_notif: std::sync::mpsc::Sender<()>, recv_frame: std::sync::mpsc::Sender<RawFrame>, dev: &MinDevice, uvc_dev: uvc::Device<'static>) -> Option<std::thread::JoinHandle<()>> {
        let cam_thread = CameraThread::init(cam_cmds, frame_notif, recv_frame, self.logging.clone(), uvc_dev, dev.index.clone());
    ...
    #[inline]
    fn new_transcoding_thread(&self, enc_cmds: std::sync::mpsc::Receiver<EncodingCommand>, frame_notif: std::sync::mpsc::Sender<()>, send_frame: std::sync::mpsc::Receiver<RawFrame>, dev: &MinDevice) -> Option<std::thread::JoinHandle<()>> {
        let enc_thread = EncoderThread::init(enc_cmds, frame_notif, send_frame, self.logging.clone(), self.encoding.clone(), &dev.index);
    ...
    fn request_open_pipeline(&mut self, dev: MinDevice, uvc_dev: uvc::Device<'static>) {
        ...
        let cam_handle = self.new_camera_thread(close_cam_recv, cam_notif_send, frame_send, &dev, uvc_dev);
        let enc_handle = self.new_transcoding_thread(close_enc_recv, enc_notif_send, frame_recv, &dev);

        if let Some(pipeline) = self.current_pipelines.get_mut(idx) {
            *pipeline = PipelineManager::instantiate(close_cam_send, close_enc_send, cam_handle, enc_handle, cam_notif_recv, enc_notif_recv);
            self.log(format!("pipeline already exists with idx {:?}, replacing the current manager", idx));
        } else {
            self.current_pipelines.insert(dev.index, PipelineManager::instantiate(close_cam_send, close_enc_send, cam_handle, enc_handle, cam_notif_recv, enc_notif_recv));
        }
    }
    ...
    fn request_close_pipeline(&mut self, dev: MinDevice) -> () {
        if let Some(mut pipeline) = self.current_pipelines.remove(dev.idx()) {
            ...
            self.log(format!("pipeline with idx {:?} does not exist within the map despite being disconnected", dev.idx()))
        }
    }
...
fn is_special_cam(dev: &rusb::DeviceDescriptor, int: &rusb::InterfaceDescriptor) -> bool {
    dev.vendor_id() == 0x199e && (dev.product_id() == 0x8102 || dev.product_id() == 0x8102) && (int.class_code() == 255 && int.sub_class_code() == 2)
}
...
impl MinDevice {
    fn from_libusb_device<T>(device: rusb::Device<T>) -> Option<Self> where T: rusb::UsbContext {
        ...
        for interface in config.interfaces() {
            for alt_setting in interface.descriptors() {
                if is_special_cam(&descriptor, &alt_setting) || is_cam(&alt_setting) {
                    return Some(Self {
                        index: DeviceIndex((device.bus_number() as u16) | ((device.address() as u16) << 8)),
...
fn is_tis_cam(descriptor: &rusb::DeviceDescriptor) -> bool {
    descriptor.vendor_id() == 0x199e && descriptor.product_id() >= 0x8201 && descriptor.product_id() <= 0x8208 
}
...
impl <T> rusb::Hotplug<T> for IoThread where T: rusb::UsbContext {
    fn device_arrived(&mut self, device: rusb::Device<T>) {

        if let Some(dev) = MinDevice::from_libusb_device(device) {
            refresh_context(uvc::Context::new().unwrap());
            let uvc_dev = get_context().find_device(dev.vendor_id, dev.product_id, None).unwrap();
            let ev = DeviceIoEvent::from_min_device(dev, DeviceIoEventKind::Joined(uvc_dev));
            self.log(&ev);
            self.send_min_event(MinIoEvent{index: ev.idx().clone(), kind: MinIoEventKind::Joined});
            self.send_event(ev);
        }
    }
...
impl IoThread {
    ...
    fn check<T>(&self, device: rusb::Device<T>, event_kind: DeviceIoEventKind) where T: rusb::UsbContext {
        if let Some(dev) = MinDevice::from_libusb_device(device) {
            if let Some(min_ev) = MinIoEventKind::from_device_event(&event_kind) {
                self.send_min_event( MinIoEvent { index: dev.idx().clone(), kind: min_ev } )
            }
    ...
    const fn init(logging: std::sync::mpsc::Sender<String>, device_event: std::sync::mpsc::Sender<DeviceIoEvent>, min_io: std::sync::mpsc::Sender<MinIoEvent>) -> Self {
        Self {
            logging,
            device_event,
            min_event: min_io,
    ...
    fn spin(self) {
        ...
        let mut registration = Some(rusb::HotplugBuilder::new().enumerate(true).register(ctx.clone(), Box::new(self)).unwrap());
...
impl Time {
    #[inline]
    fn now() -> Self {
        Time(std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap())
    }
